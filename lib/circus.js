...
var Channel = function (name, connection) {
    ...
    this.signals = {};
    this.signals.messaged = new signals.Signal;
...
Connection.prototype._handlePrivMsg = function (message) {
    ...
    if (message.middle[0] === this._actualNick) {
        this.signals.messaged.dispatch(message.trailing, agent);
    }
    else {
        ...
        channel.signals.messaged.dispatch(message.trailing, agent, channel);
    }
};
